...
int main() {
	...
	std::cin >> xMax;
	if (xMax < xMin) {
		std::cout << "Неверно введены максимальое и минимальное значения";
		return 0;
	}
	int intervals_size;
	...
	if (intervals_size >= std::abs(xMax - xMin)) {
		std::cout << "Неверно введено количество интервалов.\nКоличество интервалов должно быть меньше длинны диапозонов возможных значений.";
		return 0;
	}
	...
	if (intervals_size > 0 and left_boarders[0] < xMin) {
		std::cout << "Некоторые левые границы интервалов меньше минимального вохможного значения";
		return 0;
	}

	std::random_device rd;
	std::mt19937 gen(rd()); // генератор псевдо случайных чисел 
	...
	std::cout << "Номер интервала \tЛевая границца интервала \tКоличество чисел в интервале" << '\n';
